...
def step(pt_id, time):
    ...
    if np.abs(timeslice[0]) >= np.abs(wall_x(time)):
        ...
        t_col = brenth(f, time - dt, time, maxiter = 50)
        # calculate collision position and momentum
        x_col = particle[0](t_col)
        p_col = 2 * np.sign(x_col) * wall_v(t_col) - particle[1](t_col)
        # build new particle
        particle = (build_pos(x_col, p_col, t_col), 
                        build_mom(x_col, p_col, t_col), pt_id)
        pt_arr[pt_id] = particle
